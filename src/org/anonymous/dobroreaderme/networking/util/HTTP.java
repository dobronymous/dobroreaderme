...
public class HTTP {
    ...
    public static void writeCookies(HttpConnection c, String cookies) throws IOException {
        c.setRequestProperty("Cookie", cookies);
    }
    
    public static void readCookies(HttpConnection c) throws IOException {
        int i = 0;
        String header = "";
        while ((header = c.getHeaderField(i)) != null) {
            if (c.getHeaderFieldKey(i).equals("Set-Cookie")) {
                Settings.setCookie(header);
            }
            i++;
        }
    }
